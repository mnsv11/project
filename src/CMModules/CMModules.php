    ...
    while (($module = $dir->read()) !== false) {
      if(is_dir("$src/$module")) {
        if(class_exists($module)) {
          ...
          $modules[$module]['isLydiaCMF']    = in_array($rc->name, array('CForm', 'CCPage','CCBlog','CMUser', 'CCUser', 'CMContent', 'CCContent','CMPics','CCPics','CMForum','CCForum', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier'));
        }
      }
    }
    $dir->close();
